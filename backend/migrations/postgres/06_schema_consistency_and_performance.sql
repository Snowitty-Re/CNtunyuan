-- ============================================================
-- 助力团圆志愿者系统 - 结构一致性与性能优化（PostgreSQL）
-- 目标：
...
-- 5) 高频查询复合索引
CREATE INDEX IF NOT EXISTS idx_users_org_status_created_at
  ON ty_users(org_id, status, created_at DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_users_role_status_created_at
  ON ty_users(role, status, created_at DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_missing_persons_status_urgency_created_at
  ON ty_missing_persons(status, urgency, created_at DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_tasks_status_created_at
  ON ty_tasks(status, created_at DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_tasks_org_status_created_at
  ON ty_tasks(org_id, status, created_at DESC)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_tasks_assignee_status_created_at
  ON ty_tasks(assignee_id, status, created_at DESC)
  WHERE deleted_at IS NULL;
