...
func NewSystemConfigHandlerWithAuthz(authzService *appservice.AuthorizationService, db *gorm.DB) *SystemConfigHandler {
	return &SystemConfigHandler{authzService: authzService, db: db}
}
...
func (h *SystemConfigHandler) UpdateConfig(c *gin.Context) {
	...
	for key, raw := range req.Config {
		...
		flat[key] = config.CoerceValue(raw, current)
	}

	newCfg, _, err := config.SaveRuntimeOverrides(c.Request.Context(), h.db, cfg, flat)
	if err != nil {
		response.InternalServerError(c, "failed to save runtime settings")
		return
	}
	config.SetConfig(newCfg)

	masked := config.FlattenConfig(newCfg)
	for _, key := range sensitiveKeys() {
	...
	response.SuccessWithMessage(c, "配置已保存到数据库覆盖层（重启后完全生效）", gin.H{
		"config":           masked,
		"sensitive_fields": sensitiveKeys(),
		"storage":          "database_overrides",
	})
}
